...
public class CdfStyles {
  ...
  public Object liststyles() throws DashboardDesignerException {
    ...
    Style style = null;
    style = new Style( osgiEnvironment.getPluginSystemReader(), SYSTEM_RESOURCE_STYLES_DIR, null );
    styles.add( style );
    style = new Style( osgiEnvironment.getPluginRepositoryReader(), RESOURCE_STYLES_DIR_SOLUTION, null );
    styles.add( style );

    /*PluginsAnalyzer pluginsAnalyzer =
            new PluginsAnalyzer( CdeEngine.getEnv().getContentAccessFactory(), PentahoSystem.get( IPluginManager.class ) );
    ...
    for ( PluginsAnalyzer.PluginWithEntity entity : entities ) {
      ...
      IReadAccess access = CdeEngine.getEnv().getOtherPluginSystemReader( pluginId );
  ...
  public String getResourceLocation( String style ) {
    ...
    if ( split.length > 1 ) {
      ...
      if ( osgiEnvironment.getPluginRepositoryReader().fileExists( customStylePath ) ) {
        stylePath = customStylePath;

      } else if ( osgiEnvironment.getPluginSystemReader( SYSTEM_RESOURCE_STYLES_DIR ).fileExists( styleFilename ) ) {
